...
**このパッケージは依存を持たない。** 取り方の既定実装は同梱しない。同梱すると、
それを使わない利用者の依存解決まで縛るため — SPM の依存解決はパッケージ単位で、
ターゲット単位ではないので、使わないターゲットのために宣言した依存も利用者の解決空間に入る。
``ImageTransport`` は 3 メソッドなので、アプリが既に持っている HTTP スタックの上に書けば
30 行に満たない。
...
メタデータ API が公開 URL を返す形のバックエンドなら、`fetch(id:)` の中を
「メタデータを引く → その URL からバイト列を取る」の 2 段階にする。
返すのはどちらの場合も `Data` なので、``ImageLibrary`` から先は同じように動く。
